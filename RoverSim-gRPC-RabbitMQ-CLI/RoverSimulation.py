...
from client import publish_message_and_wait_for_reply

#final 3
TOTAL_ROVERS = 10
MINE_TXT_FILE_LOCATION = 'C:\\Users\\starr\\Downloads\\mines.txt'
MAP_TXT_FILE_LOCATION = 'C:\\Users\\starr\\Downloads\\map1.txt'
...
def find_valid_pin(rover_id,serial_number, pin_length=6):
    ...
    for pin_tuple in itertools.product(char_set, repeat=pin_length):
        pin = ''.join(pin_tuple)
        #print(f"Rover {rover_id} trying pin {pin}")
                ...
                if self.check_mine():
                    for mine in mines:
                        if mine.x == self.x and mine.y == self.y:
                            self.mine_disarm_info += publish_message_and_wait_for_reply(mine.mine_message(self.id),"Demine_Queue")+"\n"
                    #do not change this
                    self.map[self.x][self.y] = '*'
                    isMineActive = False
            elif move == 'R':
